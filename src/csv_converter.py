...
def parse_criteria_file(content: str) -> Tuple[List[str], List[str]]:
    ...
    for line in lines:
        ...
        if "INCLUSION CRITERIA" in line_stripped.upper():
            ...
            "NOTES"
        ) or line_stripped.upper().startswith("DEFINITIONS"):
...
def build_note_from_csvs(csv_files: Dict[str, pd.DataFrame]) -> str:
    ...
    if main_df is not None and not main_df.empty:
        ...
        if (
            pd.notna(pregnancy)
            and pregnancy
            and pregnancy.lower() not in ["n/a", "na", ""]
        ):
        ...
        cv_event = row.get("recent_cardiovascular_event_6m") or row.get(
            "recent_cardiovascular_event_3m"
        )
        ...
        if pd.notna(trial) and str(trial).lower() == "yes":
            note_parts.append(
                "Participation in interventional trial within last 3 months"
            )
...
def convert_csv_files_to_patient_data(
    csv_files: Dict[str, bytes], criteria_content: str
) -> Dict:
    """
