...
def _build_criteria_matrix(patients: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    Returns:
        ...
        - Остальные столбцы: полный текст критерия (заголовок), значения MET/NOT MET/UNKNOWN
    """
    if not patients:
        return pd.DataFrame()
...
def _group_patients_by_study(detailed_results: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    """
