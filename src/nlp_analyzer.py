...
class MedicalNLPAnalyzer:
    ...
    def _expand_medical_terms(self, text: str) -> str:
        ...
        synonyms = {
            ...
            "sglt2": (
                "sglt2 inhibitor gliflozin empagliflozin dapagliflozin canagliflozin"
            ),
    ...
    def _check_affirmation(self, sentence: str, criterion_text: str) -> bool:
        ...
        if not has_keyword_match:
            # Частичные совпадения (напр. "cardiomyopathy" содержит "cardio")
            criterion_words = [
                w for w in re.findall(r"\w+", criterion_lower) if len(w) > 4
            ]
    ...
    def _check_direct_exclusion_affirmation(
        self, criterion_text: str, note_sentences: List[str]
    ) -> bool:
        """
                    ...
                    for pattern in patterns:
                        if re.search(pattern, sent_lower):
                            ...
                            negation_before = re.search(
                                r"\b(?:no|not|without|denies?|never)\b.*" + pattern,
                                sent_lower,
                            )
    ...
    def _check_keyword_match(
        ...
        for sent in note_sentences:
            ...
            if matches >= 2 or (
                len(criterion_words) > 0 and matches / len(criterion_words) >= 0.5
            ):
                return (True, sent)
    ...
    def _check_numeric_criterion(
        ...
        if lvef_criterion:
            ...
            for sentence in note_sentences:
                ...
                lvef_pattern = (
                    r"(?:lvef|left\s+ventricular\s+ejection\s+fraction"
                    r"|ejection\s+fraction|ef)\s*(?:of\s+|is\s+|was\s+|:?\s*)"
                    r"(\d+)\s*%?"
                )
                lvef_match = re.search(lvef_pattern, sentence_lower)
                if lvef_match:
        ...
        if age_range:
            ...
            for sentence in note_sentences:
                ...
                if age_match:
                    ...
                    # Для диапазона: если actual вне диапазона, НЕ выполнен
        ...
        if bp_criterion:
            ...
            for sentence in note_sentences:
                ...
                # Паттерн 1: BP 120/80
                bp_match = re.search(
                    r"(?:bp|blood\s+pressure)[^0-9]*(\d+)/(\d+)", sentence_lower
                )
                if bp_match:
                ...
                sbp_match = re.search(
                    r"\bsbp\s*(?:of\s+|is\s+|:)?\s*(\d+)", sentence_lower
                )
                if sbp_match:
                ...
                systolic_match = re.search(
                    r"systolic\s*(?:bp\s*)?(?:of\s+|is\s+|:)?\s*(\d+)", sentence_lower
                )
                if systolic_match:
        ...
        if (
            "hepatic" in criterion_lower
            or "alt" in criterion_lower
            or "ast" in criterion_lower
        ):
            if "3" in criterion_lower and "uln" in criterion_lower:
                ...
                for sentence in note_sentences:
                    sentence_lower = sentence.lower()
                    if re.search(r"\balt[:\s]*\d+", sentence_lower) or re.search(
                        r"\bast[:\s]*\d+", sentence_lower
                    ):
                        return (False, sentence)

        return (None, None)

    def _check_statin_initiation(
        self, note_sentences: List[str]
    ) -> Tuple[Optional[bool], Optional[str]]:
        """
        ...
        statin_group = "|".join(statin_names)
        initiation_patterns = [
            r"\b(?:start|started|starting|begin|began|initiat\w+|prescrib\w+|"
            r"order\w+)\s+(?:\w+\s+)*?(" + statin_group + r")",
            r"\bplan[:\s]+(?:start|initiat\w+|begin)\s+(?:\w+\s+)*?("
            + statin_group + r")",
            r"\b(" + statin_group + r")\s+(?:\d+\s*mg\s+)?"
            r"(?:daily|nightly|qd|qhs)\b.*(?:start|begin|initiat)",
        ]
        ...
        for sentence in note_sentences:
            ...
            if any(
                statin in sentence_lower for statin in statin_names[:7]
            ):  # Конкретные статины
                if any(
    ...
    def check_negation_in_context(
        self, sentence: str, criterion_keywords: List[str], criterion_text: str = ""
    ) -> bool:
        """
        Проверяет наличие отрицания в контексте ключевых слов критерия.
        ВАЖНО: negation должен предшествовать keyword в предложении.
        """
        sentence_lower = sentence.lower()
        criterion_lower = criterion_text.lower()
        ...
        strong_negation_phrases = [
            ...
            r"\bnot\s+(?:currently\s+)?(?:in|enrolled\s+in)\s+(?:a\s+)?"
            r"(?:clinical\s+)?trial\b",
        ...
        affirmation_phrases = [
            ...
            r"currently\s+(?:on|taking)\s+(?:\w+\s+)*?"
            r"(?:empagliflozin|dapagliflozin|canagliflozin)",
        ]
        ...
        if "hypotension" in criterion_lower or "bp" in criterion_lower:
            bp_match = re.search(
                r"\b(?:bp|blood\s+pressure)\s*(?:is\s+)?(\d+)/(\d+)", sentence_lower
            )
            if bp_match:
                systolic = int(bp_match.group(1))
                if systolic >= 95:
        ...
        # Применяются если в предложении есть связанный keyword
    ...
    def analyze_criterion_against_note(
        ...
        numeric_result, numeric_sentence = self._check_numeric_criterion(
            criterion.text, note_sentences
        )
        if numeric_result is not None:
            ...
            return {
                ...
                "has_negation": False,
                "explanation": (
                    f"Numeric criterion: {'MET' if numeric_result else 'NOT MET'}"
                ),
            }

        # Специальная проверка для statin initiation (Study W02)
        criterion_lower = criterion.text.lower()
        if "statin" in criterion_lower and (
            "initiation" in criterion_lower or "new" in criterion_lower
        ):
            statin_result, statin_sentence = self._check_statin_initiation(
                note_sentences
            )
            if statin_result is not None:
                return {
                    ...
                    "matched_sentence": statin_sentence or "[statin check]",
                    "has_negation": False,
                    "explanation": (
                        f"Statin initiation: {'MET' if statin_result else 'NOT MET'}"
                    ),
                }
        ...
        if criterion.type == "exclusion":
            ...
            # Прямое утверждение для exclusion критериев
            # MET, независимо от negation в других предложениях
            direct_affirmation = self._check_direct_exclusion_affirmation(
                criterion.text, note_sentences
            )
            ...
            has_negation = False
            for sent in note_sentences:
                if self.check_negation_in_context(
                    sent, criterion.keywords, criterion.text
                ):
                    has_negation = True
                    break
            ...
            has_negation = self.check_negation_in_context(
                best_sentence, criterion.keywords, criterion.text
            )
    ...
    def evaluate_patient(self, patient: PatientData) -> Dict[str, Any]:
        ...
        expanded_sentences = [
            self._expand_medical_terms(s) for s in patient.note_sentences
        ]
        sentence_embeddings = self.encode_texts(expanded_sentences)
        ...
        inclusion_not_met_count = sum(
            1 for r in inclusion_results if r["is_met"] is False
        )
        inclusion_unknown_count = sum(
            1 for r in inclusion_results if r["is_met"] is None
        )
        total_inclusion = len(inclusion_results)

        exclusion_met_count = sum(1 for r in exclusion_results if r["is_met"] is True)
        exclusion_unknown_count = sum(
            1 for r in exclusion_results if r["is_met"] is None
        )
        ...
        # 1. Exclusion met → excluded
        # 2. Inclusion NOT met → excluded
        # 3. >20% unknown → not enough information
        # 4. ≤20% unknown + met inclusion → included
        if exclusion_met_count > 0:
            ...
            predicted_status = "excluded"
            summary = (
                f"EXCLUDED: {inclusion_not_met_count} inclusion criteria not met"
            )
            ...
            unknown_details = "; ".join(unknown_criteria[:3])  # Первые 3
            if len(unknown_criteria) > 3:
                unknown_details += f" (+{len(unknown_criteria) - 3} more)"
            summary = (
                f"NEEDS CLARIFICATION: {total_unknown}/{total_criteria} "
                f"({unknown_ratio:.0%}) criteria unknown — {unknown_details}"
            )
            ...
            if total_unknown > 0:
                summary = (
                    f"INCLUDED: {inclusion_met_count}/{total_inclusion} inclusion met, "
                    f"{total_unknown} unknown (≤{UNKNOWN_THRESHOLD:.0%})"
                )
            else:
                summary = (
                    f"INCLUDED: {inclusion_met_count}/{total_inclusion} inclusion met, "
        ...
        if patient.ground_truth:
            ...
            if "include" in gt_normalized and predicted_status == "included":
                ...
                "not enough" in gt_normalized or "info" in gt_normalized
            ) and predicted_status == "not enough information":
                is_correct = True
